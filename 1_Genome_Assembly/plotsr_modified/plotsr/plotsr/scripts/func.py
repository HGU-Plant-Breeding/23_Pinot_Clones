...
# MODIFICATION START
def calculate_snp_density(snp_file, window_size, chrlengths):
    """
    Reads a SNP file and calculates the number of SNPs in defined windows.
    :param snp_file: Path to the SNP file (format: GenID\tChrID\tPos)
    :param window_size: Size of the window in base pairs.
    :param chrlengths: Data structure containing chromosome lengths.
    :return: A dictionary containing SNP counts per window for each chromosome.
    """
    logger = logging.getLogger('calculate_snp_density')
    density_data = {}

    # Initialize the data structure
    for genome_id, chr_map in chrlengths:
        density_data[genome_id] = {}
        for chr_id, length in chr_map.items():
            num_windows = math.ceil(length / window_size)
            density_data[genome_id][chr_id] = [0] * num_windows

    # Read the SNP file and populate counts
    with open(snp_file, 'r') as fin:
        for line in fin:
            ...
            genome_id, chr_id, pos = parts[0], parts[1], int(parts[2])

            if genome_id in density_data and chr_id in density_data[genome_id]:
                # Check if position is within chromosome bounds
                chr_len = next(cl[1][chr_id] for cl in chrlengths if cl[0] == genome_id)
                if pos >= chr_len:
                    logger.warning(f"SNP position {pos} is outside the length of chromosome {chr_id} ({chr_len} bp) in genome {genome_id}. Skipping.")
                    continue
                
                bin_index = pos // window_size
                if bin_index < len(density_data[genome_id][chr_id]):
                    density_data[genome_id][chr_id][bin_index] += 1
                else:
                    logger.warning(f"Calculated bin index {bin_index} is out of bounds for chromosome {chr_id} in genome {genome_id}. This may indicate an issue with chromosome length or SNP position. Skipping SNP at pos {pos}.")
            else:
                # Log this only once per missing chromosome to avoid spamming
                if not hasattr(calculate_snp_density, "warned"):
                    calculate_snp_density.warned = set()
                if (genome_id, chr_id) not in calculate_snp_density.warned:
                    logger.warning(f"SNP entry for genome '{genome_id}', chromosome '{chr_id}' will be ignored as it's not in the provided genomes/chromosomes list.")
                    calculate_snp_density.warned.add((genome_id, chr_id))


    return density_data

def get_density_statistics(density_data, percentile_value):
    """
    Calculates the global normalization value based on a percentile.
    :param density_data: The dictionary of SNP counts from calculate_snp_density.
    :param percentile_value: The percentile to use for clamping (0-100).
    :return: The SNP count value at the specified percentile.
    """
    all_counts = []
    for genome in density_data.values():
        for chr_counts in genome.values():
            all_counts.extend(chr_counts)

    if not all_counts:
        return 1.0  # Return a default value if there are no SNPs

    if percentile_value == 100:
        return float(np.max(all_counts))
        
    norm_max = np.percentile(all_counts, percentile_value)
    
    # If the percentile is 0 (e.g., very sparse data), use the max value instead
    if norm_max == 0 and np.max(all_counts) > 0:
        return float(np.max(all_counts))

    return float(norm_max) if norm_max > 0 else 1.0


def draw_heatmap_bar(ax, density_list, chr_len, window_size, bar_pos, bar_thickness, vertical, colormap_name, norm_max):
    """
    Draws a single chromosome bar as a heatmap.
    :param ax: Matplotlib axis object.
    :param density_list: List of SNP counts for the chromosome.
    :param chr_len: The total length of the chromosome.
    :param window_size: The window size used for density calculation.
    :param bar_pos: The y-position (for horizontal) or x-position (for vertical) of the bar.
    :param bar_thickness: The thickness of the heatmap bar.
    :param vertical: Boolean, True if plot is vertical.
    :param colormap_name: Name of the matplotlib colormap.
    :param norm_max: The maximum value for the color scale normalization.
    """
    cmap = cm.get_cmap(colormap_name)
    normalizer = mcolors.Normalize(vmin=0, vmax=norm_max)

    for i, count in enumerate(density_list):
        start_pos = i * window_size
        # Ensure the last window doesn't exceed the chromosome length
        win_len = min(window_size, chr_len - start_pos)
        
        # Clamp count to the normalization max
        color_val = min(count, norm_max)
        color = cmap(normalizer(color_val))

        if not vertical:
            # (x, y), width, height
            rect = patches.Rectangle((start_pos, bar_pos - (bar_thickness / 2)), win_len, bar_thickness,
                                     linewidth=0, facecolor=color, zorder=2)
        else:
            # (x, y), width, height
            rect = patches.Rectangle((bar_pos - (bar_thickness / 2), start_pos), bar_thickness, win_len,
                                     linewidth=0, facecolor=color, zorder=2)
        ax.add_patch(rect)
...
################################################################################
"""
# ... (rest of the original file from here)
def readbasecfg(f, v):
    import logging
    import matplotlib
...
def pltchrom(ax, chrs, chrgrps, chrlengths, v, S, genomes, cfg, itx, minl=0, maxl=-1,
             snp_density_data=None, snp_window_size=None, snp_colormap=None, snp_norm_max=None, snp_bar_thickness=0.08):
# MODIFICATION END
    ...
    if not itx:
        ...
        for s in range(len(chrlengths)):
            for i in range(len(chrs)):
                ...
                current_chr_id = chrgrps[chrs[i]][s]
                current_genome_id = chrlengths[s][0]

                if maxl == -1:
                    maxcoord = chrlengths[s][1][current_chr_id]
                else:
                    maxcoord = maxl
                
                genome = [gen for gen in genomes if gen.n == current_genome_id][0]

                # MODIFICATION START
                # Decide whether to draw a heatmap or a simple line
                if snp_density_data:
                    density_list = snp_density_data[current_genome_id][current_chr_id]
                    bar_thickness = snp_bar_thickness  # Adjust this factor as needed
                    draw_heatmap_bar(ax, density_list, maxcoord, snp_window_size,
                                     indents[s]-offset, bar_thickness, v, snp_colormap, snp_norm_max)
                    # For the legend, we still want a simple line handle
                    if not chrlabs[s]:
                         # Create a proxy artist for the legend
                        proxy_line = plt.Line2D([0], [0], color=genome.lc, lw=genome.lw, label=current_genome_id)
                        chrlabels.append(proxy_line)
                        chrlabs[s] = True
        ...
        for s in range(len(chrlengths)):
            ...
            for i in range(len(chrs)):
                ...
                # MODIFICATION START
                if snp_density_data:
                    current_chr_id = chrgrps[chrs[i]][s] if not v else chrgrps[chrs[len(chrs)-1-i]][s]
                    current_genome_id = chrlengths[s][0]
                    density_list = snp_density_data[current_genome_id][current_chr_id]
                    chr_len_for_heatmap = end - start
                    bar_thickness = genome.lw / 20 

                    # We need to adapt draw_heatmap_bar slightly for ITX, or create a temporary one.
                    # For simplicity, let's adapt it here. The principle is the same.
                    cmap = cm.get_cmap(snp_colormap)
                    normalizer = mcolors.Normalize(vmin=0, vmax=snp_norm_max)
                    num_windows = math.ceil(chr_len_for_heatmap / snp_window_size)
                    
                    for k in range(num_windows):
                        win_start = k * snp_window_size
                        win_len = min(snp_window_size, chr_len_for_heatmap - win_start)
                        color_val = min(density_list[k], snp_norm_max)
                        color = cmap(normalizer(color_val))
                        
                        if not v:
                            rect = patches.Rectangle((start + win_start, fixed - (bar_thickness / 2)), win_len, bar_thickness,
                                                     linewidth=0, facecolor=color, zorder=2)
                        else:
                            rect = patches.Rectangle((fixed - (bar_thickness / 2), start + win_start), bar_thickness, win_len,
                                                     linewidth=0, facecolor=color, zorder=2)
                        ax.add_patch(rect)
